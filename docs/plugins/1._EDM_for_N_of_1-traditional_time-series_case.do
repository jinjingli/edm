* CITE AS: Zyphur, M. J., & Li, J. (2019). EDM for N of 1. Stata .do file retrieved online.
* CONTACT: mzyphur@unimelb.edu.au

* NOTES ON THE .DO FILE AND RESULTS:
* This runs EDM analyses for N=1 time-series data (tsset) as follows:
...
* SET VARIABLE AND TIME VARIABLE INFO
...
local timevar = ""   // Name of tsset time variable entered in quotes
...
replace ``var'_nonzeroSP' = 0 if `var' == 0
replace ``var'_nonzeroSP' = sum(``var'_nonzeroSP')
replace ``var'_nonzeroSPuse' = 1 if ``var'_nonzeroSP'[_n] < ``var'_nonzeroSP'[_n-`emaximum'-`diff']
tsset
...
di "-------------------------------"
capture noisily {
edm explore `prestd'd`dval'.`var' if `panelvar'==`id' ``var'dropzesSP', e(`eminimum'/`emaximum') `SPsampler'(`SPreps') `fully' `forcing' `time_gap' `missings' extra(`extras`dval'' `extras_raw') `tweight' `mdistance' reportrawe `options'
}
capture {
...
replace ``var'_nonzero' = 0 if `var' == 0
replace ``var'_nonzero' = sum(``var'_nonzero')
sort `timevar'
foreach id of numlist `panelrange' {
replace ``var'_nonzerouse' = 1 if ``var'_nonzero'[_n] < ``var'_nonzero'[_n-`emax`var'`id''-`diff']
}
tsset
...
if "`dropzeros_SP'"=="yes" {
bysort ``var'_nonzeroSPuse' (`randoM'): replace `newtimE'=_n if ``var'_nonzeroSPuse'==1
bysort ``var'_nonzeroSPuse' (`timevar'): replace `r`var''=`d`diff'`var''[`newtimE'] if ``var'_nonzeroSPuse'==1
tsset
}
else {
sort `randoM'
replace `newtimE'=_n
sort `timevar'
replace `r`var''=`d`diff'`var''[`newtimE']
tsset
}
foreach id of numlist `panelrange' {
if "`emax`var'`id''"!="" {
...
di "--------------------------------"
capture noisily {
edm explore `prestd'd`diff'.`var' if `panelvar'==`id' ``var'dropzes', e(`emax`var'`id'') `TPsampler'(`TPreps') `fully' tp(`i') `forcing' `time_gap' `missings' extra(`extras`diff'' `extras_raw') `tweight' `mdistance' reportrawe `options'
}
capture {
mata: `D'=`i'
...
/*xlab(`panelrange')*/ name(SMAPtest_`var'_`testval'1_d`diff', replace) yline(0) `notess' graphregion(color(white)) `savers1' xline(`panelrange', lwidth(vthin) lpattern(dash))
...
/*xlab(`panelrange')*/  name(SMAPtest_`var'_`testval'2_d`diff', replace)  `notess' `savers2' xline(`panelrange', lwidth(vthin) lpattern(dot))
...
di "Below CCM Cause==`var1', Outcome==`var2'"
di "-------------------------------------------------"
edm xmap `prestd'd`diff'.`var2' `prestd'd`diff'.`var1' if `panelvar'==`id' ``var1'dropzes1', e(`emax`var1'`id'') oneway lib(`lib11') rep(`CCMreps1') `forcing' `time_gap' `missings' extra(`extras`diff'' `extras_raw') `tweight' `mdistance' `options'
}
capture {
...
di "Below CCM Cause==`var1', Outcome==`var2'"
di "-------------------------------------------------"
edm xmap `prestd'd`diff'.`var2' `prestd'd`diff'.`var1' if `panelvar'==`id' ``var1'dropzes1', e(`emax`var1'`id'') oneway lib(`lib22' `lib33' `lib44') rep(`CCMreps2') `forcing' `time_gap' `missings' extra(`extras`diff'' `extras_raw') `tweight' `mdistance' `options'
}
capture {
...
di "Below CCM Cause==`var1', Outcome==`var2'"
di "-------------------------------------------------"
edm xmap `prestd'd`diff'.`var2' `prestd'd`diff'.`var1' if `panelvar'==`id' ``var1'dropzes1', e(`emax`var1'`id'') oneway lib(`libthird' `libhalf') rep(`CCMreps3') `forcing' `time_gap' `missings' extra(`extras`diff'' `extras_raw') `tweight' `mdistance' `options'
}
capture {
...
di "-------------------------------------------------------------------------------------------"
capture {
edm xmap `prestd'd`diff'.`var2' `prestd'd`diff'.`var1' if `panelvar'==`id' ``var1'dropzes1', e(`emax`var1'`id'') oneway lib(`lib11') rep(`newCCMctestreps') `forcing' `time_gap' `missings' extra(`extras`diff'' `extras_raw') `tweight' `mdistance' `options'
}
capture {
...
replace `randoM'=uniform()
if "`dropzeros'"=="yes" {
bysort ``var1'_nonzerouse' (`randoM'): replace `newtimE'=_n if ``var1'_nonzerouse'==1
bysort ``var1'_nonzerouse' (`timevar'): replace `r`var1''=`d`diff'`var1''[`newtimE'] if ``var1'_nonzerouse'==1
tsset
}
else {
sort `randoM'
replace `newtimE'=_n
sort `timevar'
replace `r`var1''=`d`diff'`var1''[`newtimE']
tsset
}
...
di "--------------------------------------------------------------------------------"
capture {
qui edm xmap `prestd'd`diff'.`var2' `prestd'`r`var1'' if `panelvar'==`id' ``var1'dropzes1', e(`emax`var1'`id'') oneway `forcing' `time_gap' `missings' extra(`extras`diff'' `extras_raw') `tweight' `mdistance' `options'
}
capture {
mata: `Z21' = (`id')
mata: `Z1'=st_matrix("e(xmap_1)")
