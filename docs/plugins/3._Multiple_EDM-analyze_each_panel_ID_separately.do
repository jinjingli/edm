* CITE AS: Zyphur, M. J., & Li, J. (2019). Multiple EDM (analyze each panel ID separately). Stata .do file retrieved online.
* CONTACT: mzyphur@unimelb.edu.au

* NOTES ON THE .DO FILE AND RESULTS:
* This estimates 'multiple EDM' for each ID of panel data (xtset) as follows:
...
local panelvar = ""   // Name of xtset panel ID variable entered in quotes
local panelrange = ""   // Range of panel IDs to analyze (as a numlist of values on the panel ID variable; e.g., "1/50")
* Note 1: Non-sequential panel ID values can be resolved with "egen newid = group(oldid)" and using the "newid" variable
* Note 2: For the N=1 case a panelvar must be specified that has a single value for all observations, with the value entered as panelrange
* Note 3: For example the N=1 case can be set as "gen ID = 1", then ID is the panelvar and 1 is the panelrange setting
...
local grandmeanonly = "no"   // Set "yes" or "no" to plot only the grand mean
* Note 1: Set "yes" for the N=1 case or when too many panels (>100) cause clutter
* Note 2: Grand mean always gets its own plot, but it is also overlayed on plots for all panel IDs,
* so setting grandmeanonly = "yes" merely eliminates the plots that include all panel IDs
local CIs = 0   // Set 95 or 90 for 90% or 95% CI error bars; enter 0 for no CIs, which is advised with many panel IDs (N > 20)
...
local GMCIs = 95   // Set to 95 or 90 for 90% or 95% CI error bars; enter 0 for no CIs
* Note 1: This option does not affect the way hypothesis tests are plotted, and instead merely pertains to regular analysis output
* Note 2: For simplex projection, prediction step analysis (tp), and SMAPs, the grand mean CIs are for each E as sqrt(mean(SD^2 of rho/MAE))
* Note 3: CIs for convergent cross mapping (CCM) not available
...
local SPdescribe = "yes"   // Set "yes" or "no" for plots of rho-MAE relationships and E at max(rho) versus min(mae)
* Note 1: This option produces extensive diagnostic plots that may not be useful in many cases... or even comprehensible for that matter
* Note 2: Most 'SPdescribe' plots accompany 'Ecriterion'="rho" or "mae" to choose E (and thus require multiple E values analyzed)
...
if "`r(varlist)'"!="" {
*******************************************************************************************
* Note1: Sorry, you have at least one variable with a name longer than 4 characters.      *
...
tempvar tempstD tempdt1 absvaluE randoM newtimE obsnumbeR obsnO
...
qui bysort `panelvar' (`timevar'): replace ``var'_nonzeroSP' = 0 if `var' == 0
qui bysort `panelvar' (`timevar'): replace ``var'_nonzeroSP' = sum(``var'_nonzeroSP')
qui bysort `panelvar' (`timevar'): replace ``var'_nonzeroSPuse' = 1 if ``var'_nonzeroSP'[_n] < ``var'_nonzeroSP'[_n-`emaximum'-`diff']
tsset
...
label variable SP`var'1 "Simplex Projection panel ID from `panelvar'"
...
label variable SPlong`var'1 "Simplex Projection long (all `SPsampler's), panel ID from `panelvar'"
...
foreach var of varlist `variables' {
qui replace SP_id_`var' = 10^9 if _n > _N/2 & !missing(SP_id_`var')
}
foreach var of varlist `variables' {
* Confidence intervals for full plots
if `CIs'==95 loc zci=1.96
if `CIs'==90 loc zci=1.65
if `CIs'==95 | `CIs'==90 {
loc a="err(collapse(cilower`var' ciupper`var'))"
qui gen SP_`useresults'SD_`var'var=SP_`useresults'SD_`var'^2
qui bysort SP_id_`var' SP_d_`var' SP_e_`var': egen cilower`var' = mean(SP_`useresults'SD_`var'var)
qui bysort SP_id_`var' SP_d_`var' SP_e_`var': egen ciupper`var' = mean(SP_`useresults'SD_`var'var)
qui replace cilower`var' = SP_`useresults'_`var' - sqrt(cilower`var')*`zci'
qui replace ciupper`var' = SP_`useresults'_`var' + sqrt(ciupper`var')*`zci'
sort expanded `obsnumbeR'
}
else loc a=""
* Confidence intervals for grand means
if `GMCIs'==95 loc zgmci=1.96
if `GMCIs'==90 loc zgmci=1.65
if `GMCIs'==95 | `GMCIs'==90 {
loc gmci="err(collapse(cilower`var'gm ciupper`var'gm))"
qui gen SP_`useresults'SD_`var'vargm=SP_`useresults'SD_`var'^2
qui bysort SP_id_`var' SP_d_`var' SP_e_`var': egen cilower`var'gm = mean(SP_`useresults'SD_`var'vargm)
qui bysort SP_id_`var' SP_d_`var' SP_e_`var': egen ciupper`var'gm = mean(SP_`useresults'SD_`var'vargm)
qui replace cilower`var'gm = SP_`useresults'_`var' - sqrt(cilower`var'gm)*`zgmci'
qui replace ciupper`var'gm = SP_`useresults'_`var' + sqrt(ciupper`var'gm)*`zgmci'
sort expanded `obsnumbeR'
}
else loc gmci=""
...
* Saving results
if "`SPsave'"=="yes" {
loc b="saving(SP_`useresults'_`var'_d`dval', replace)"
loc bb="saving(SP_ScatterB_`var'_d`dval', replace)"
loc bbb="saving(SP_ScatterW_`var'_d`dval', replace)"
loc gmb="saving(SP_`useresults'_`var'_d`dval'_gm, replace)"
}
else {
loc b=""
loc bb=""
...
if "`grandmeanonly'"=="yes" {
* Plot command
lgraph SP_`useresults'_`var' SP_e_`var' SP_id_`var' if SP_d_`var'==`dval' & SP_id_`var'==1000000000, ///
`gmci' `gmb' separate(.001) fit(`SPlinefit') `ylabelscale' ///
title({stSerif:SP: Grand-Mean Results for `var'}, color(black)) ///
...
loptions(1000000000 lwidth(*6)) foptions(1000000000 lwidth(*6)) graphregion(color(white)) bgcolor(white) ///
name(SP_`useresults'_`var'_d`dval'_gm, replace) nolegend

if "`exportplot'" != "" {
graph export SP_`useresults'_`var'_d`dval'_gm.`exportplot', replace
}
...
* Plot command
lgraph SP_`useresults'_`var' SP_e_`var' SP_id_`var' if SP_d_`var'==`dval', ///
...
if "`SPdescribe'"=="yes" & `eminimum' != `emaximum' {
* Regression to get Between- and Within-ID data for rho and mae
capture qui {
reg SP_mae_`var' i.SP_id_`var' if SP_d_`var'==`dval' & _n < _N/2 & !missing(SP_id_`var')
predict SP_mae_xb_`var'_d`dval' if e(sample)==1, xb
predict SP_mae_res_`var'_d`dval' if e(sample)==1, residuals
label variable SP_mae_xb_`var'_d`dval' "The between-ID part of mae from simplex projections"
label variable SP_mae_res_`var'_d`dval' "The within-ID or residual part of mae from simplex projections"
gen xb`var'd`dval'=.
replace xb`var'd`dval'=10^9 if !missing(SP_mae_xb_`var'_d`dval')
reg SP_rho_`var' i.SP_id_`var' if SP_d_`var'==`dval' & _n < _N/2 & !missing(SP_id_`var')
predict SP_rho_xb_`var'_d`dval' if e(sample)==1, xb
predict SP_rho_res_`var'_d`dval' if e(sample)==1, residuals
label variable SP_rho_xb_`var'_d`dval' "The between-ID part of rho from simplex projections"
label variable SP_rho_res_`var'_d`dval' "The within-ID or residual part of rho from simplex projections"
}
capture qui {
putmata `Bpart'=(SP_mae_xb_`var'_d`dval' SP_rho_xb_`var'_d`dval' xb`var'd`dval'), omitmissing replace
putmata `Wpart'=(SP_mae_res_`var'_d`dval' SP_rho_res_`var'_d`dval' SP_id_`var'), omitmissing replace
mata: `O'=`Wpart'\ `Bpart'
getmata (SP_mae_reg_`var'_d`dval' SP_rho_reg_`var'_d`dval' SP_tempid_`var'_d`dval')=`O', force double 
drop SP_mae_xb_`var'_d`dval' SP_rho_xb_`var'_d`dval' SP_mae_res_`var'_d`dval' SP_rho_res_`var'_d`dval'
qui cor SP_rho_reg_`var'_d`dval' SP_mae_reg_`var'_d`dval' if SP_tempid_`var'_d`dval'==1000000000
loc regrho1 = round(r(rho), .01)
loc regrho11: di %3.2f `regrho1'
}
capture {
lgraph SP_rho_reg_`var'_d`dval' SP_mae_reg_`var'_d`dval' SP_tempid_`var'_d`dval' if SP_tempid_`var'_d`dval'==1000000000, ///
title({stSerif:SP: Between-ID} {&rho}{stSerif:-MAE plot for `var'}, color(black)) ///
ytitle({&rho}) xtitle({stSerif:MAE}) fit(lfit) ///
/*loptions(1000000000 lwidth(*6)) foptions(1000000000 lwidth(*6))*/ ///
graphregion(color(white)) bgcolor(white) name(SP_ScatterB_`var'_d`dval', replace) ///
nolegend `bb' separate(.001) note("{stSerif:Note: Average} {&rho}{stSerif:-MAE relationship over all E (each point is a panel ID) r = `regrho11' -- differencing at d`dval'}")

if "`exportplot'" != "" {
graph export SP_ScatterB_`var'_d`dval'.`exportplot', replace
}

qui cor SP_rho_reg_`var'_d`dval' SP_mae_reg_`var'_d`dval' if SP_tempid_`var'_d`dval'<1000000000
loc regrho2 = round(r(rho), .01)
loc regrho22: di %3.2f `regrho2'

lgraph SP_rho_reg_`var'_d`dval' SP_mae_reg_`var'_d`dval' SP_tempid_`var'_d`dval' if SP_tempid_`var'_d`dval'<1000000000, ///
title({stSerif:SP: Within-ID} {&rho}{stSerif:-MAE plot for `var'}, color(black)) ///
ytitle({&rho}) xtitle({stSerif:MAE}) fit(lfit) ///
graphregion(color(white)) bgcolor(white) name(SP_ScatterW_`var'_d`dval', replace) ///
nolegend `bbb' separate(.001) note("{stSerif:Note:} {&rho}{stSerif:-MAE relationship at each E (removing Between-ID averages) r = `regrho22' -- differencing at d`dval'}")

if "`exportplot'" != "" {
graph export SP_ScatterW_`var'_d`dval'.`exportplot', replace
}
...
* Plots based on chosen E
if "`SPgraph'"=="yes" & "`plots_on'" =="yes" {
if "`SPdescribe'"=="yes" {
foreach var of varlist `variables' {
qui mean SP_rho_`var' if SP_d_`var'==`diff'
mat `RhoGM'=e(b)
loc rhoGM`var'=round(`RhoGM'[1,1], .01)
loc rhoGM1`var': di %3.2f `rhoGM`var''
qui mean SP_mae_`var' if SP_d_`var'==`diff'
mat `MaeGM'=e(b)
loc maeGM`var'=round(`MaeGM'[1,1], .01)
loc maeGM1`var': di %3.2f `maeGM`var''
* Saving results
if "`SPsave'"=="yes" {
loc b="saving(Histogram_chosen_rho_`var'_d`diff', replace)"
loc bb="saving(Histogram_chosen_mae_`var'_d`diff', replace)"
}
else {
loc b=""
loc bb=""
...
capture {
hist SP_rho_`var' if SP_d_`var'==`diff', percent ///
title({stSerif:SP:} {&rho} {stSerif:per Panel ID at E = `eminimum' for `var'}, color(black)) ///
ytitle({stSerif:Percent}) xtitle({&rho} {stSerif:at Chosen E = `eminimum'}) ///
note({stSerif:Note: Grand Mean r = `rhoGM1`var'' -- differencing at d`diff'}) ///
graphregion(color(white)) `b' name(Hist_chosen_rho_`var', replace) graphregion(color(white))

if "`exportplot'" != "" {
graph export Hist_chosen_rho_`var'.`exportplot', replace
}

hist SP_mae_`var' if SP_d_`var'==`diff', percent ///
title({stSerif:SP: MAE per Panel ID at E = `eminimum' for `var'}, color(black)) ///
ytitle({stSerif:Percent}) xtitle({&rho} {stSerif:at Chosen E = `eminimum'}) ///
note({stSerif:Note: Grand Mean r = `maeGM1`var'' -- differencing at d`diff'}) ///
graphregion(color(white)) `b' name(Hist_chosen_mae_`var', replace) graphregion(color(white))

if "`exportplot'" != "" {
graph export Hist_chosen_mae_`var'.`exportplot', replace
}
...
* Plots for the maximum rho or minimum MAE case
if "`SPgraph'"=="yes" & "`plots_on'" =="yes" {
if "`SPdescribe'"=="yes" {
foreach var of varlist `variables' {
qui gen emax_rho_`var'=.
qui gen emin_mae_`var'=.
qui gen max_rho_`var'=.
qui gen min_mae_`var'=.
label variable max_rho_`var' "Maximum rho per Panel ID at d`diff'"
label variable emax_rho_`var' "E of Maximum rho per Panel ID at d`diff'"
label variable min_mae_`var' "Minimum MAE per Panel ID at d`diff'"
label variable emin_mae_`var' "E of Minimum MAE per Panel ID at d`diff'"
* Saving results
if "`SPsave'"=="yes" {
loc b="saving(Scatter_maerho_`var'_d`diff', replace)"
loc bb="saving(Scatter_Emaerho_`var'_d`diff', replace)"
loc bbb="saving(Histogram_rho_`var'_d`diff', replace)"
loc bbbb="saving(Histogram_Erho_`var'_d`diff', replace)"
loc bbbbb="saving(Histogram_mae_`var'_d`diff', replace)"
loc bbbbbb="saving(Histogram_Emae_`var'_d`diff', replace)"
}
else {
loc b=""
loc bb=""
...
foreach id of numlist `panelrange' {
capture {
quietly summarize SP_rho_`var' if SP_id_`var'==`id' & SP_d_`var'==`diff', meanonly
qui replace max_rho_`var' = r(max) if SP_id_`var'==`id' & SP_d_`var'==`diff'
sca `Levelslove'=r(max)
qui levelsof `obsnO' if SP_rho_`var' == `Levelslove' & SP_id_`var'==`id' & SP_d_`var'==`diff'
qui replace emax_rho_`var' = SP_e_`var'[`r(levels)'] if SP_id_`var'==`id' & SP_d_`var'==`diff'

quietly summarize SP_mae_`var' if SP_id_`var'==`id' & SP_d_`var'==`diff', meanonly
qui replace min_mae_`var' = r(min) if SP_id_`var'==`id' & SP_d_`var'==`diff'
sca `Levelslove'=r(min)
qui levelsof `obsnO' if SP_mae_`var' == `Levelslove' & SP_id_`var'==`id' & SP_d_`var'==`diff'
qui replace emin_mae_`var' = SP_e_`var'[`r(levels)'] if SP_id_`var'==`id' & SP_d_`var'==`diff'
}
}

capture {
qui mean max_rho_`var'
mat `RhoGM'=e(b)
loc rhoGM`var'=round(`RhoGM'[1,1], .01)
loc rhoGM1`var': di %3.2f `rhoGM`var''
qui mean min_mae_`var'
mat `MaeGM'=e(b)
loc maeGM`var'=round(`MaeGM'[1,1], .01)
loc maeGM1`var': di %3.2f `maeGM`var''

qui cor max_rho_`var' min_mae_`var'
loc regrho3 = round(r(rho), .01)
loc regrho33: di %3.2f `regrho3'

qui spearman emax_rho_`var' emin_mae_`var'
loc regrho4 = round(r(rho), .01)
loc regrho44: di %3.2f `regrho4'
}

scatter max_rho_`var' min_mae_`var' || lfit max_rho_`var' min_mae_`var', ///
title({stSerif:SP: Max} {&rho} {stSerif:by Min MAE} {stSerif:per Panel ID for `var'}, color(black)) ///
ytitle({stSerif:Maximum} {&rho}) xtitle({stSerif:Minimum MAE}) ///
graphregion(color(white)) name(Scatter_maerho_`var'_d`diff', replace) ///
legend(off) lcolor(black) lwidth(.75) `b' note("{stSerif:Note: Max} {&rho}{stSerif:-min MAE relationship (each point is a panel ID) r = `regrho33' -- differencing at d`diff'}")

if "`exportplot'" != "" {
graph export Scatter_maerho_`var'_d`diff'.`exportplot', replace
}

scatter emax_rho_`var' emin_mae_`var' || lfit emax_rho_`var' emin_mae_`var', ///
title({stSerif:SP: E at Max} {&rho} {stSerif:by E of Min MAE} {stSerif:per Panel ID for `var'}, color(black)) ///
ytitle({stSerif:E at Maximum} {&rho}) xtitle({stSerif:E at Minimum MAE}) ///
graphregion(color(white)) name(Scatter_Emaerho_`var'_d`diff', replace) ///
jitter(7) legend(off) lcolor(black) lwidth(.75) `bb' note("{stSerif:Note: E at max} {&rho}{stSerif: and E at min MAE relationship (each point is a panel ID) rank r = `regrho44' -- differencing at d`diff'}")

if "`exportplot'" != "" {
graph export Scatter_Emaerho_`var'_d`diff'.`exportplot', replace
}

hist max_rho_`var', percent ///
title({stSerif:SP: Max} {&rho} {stSerif:per Panel ID for `var'}, color(black)) ///
ytitle({stSerif:Percent}) xtitle({stSerif:Maximum} {&rho}) ///
note({stSerif:Note: Grand Mean of} {&rho} {stSerif:at E That Maximizes} {&rho} {stSerif:= `rhoGM1`var'' -- differencing at d`diff'}) ///
graphregion(color(white)) `bbb' name(Hist_max_rho_`var', replace) graphregion(color(white))

if "`exportplot'" != "" {
graph export Hist_max_rho_`var'.`exportplot', replace
}

hist emax_rho_`var', percent ///
title({stSerif:SP: E at Max} {&rho} {stSerif:per Panel ID for `var'}, color(black)) ///
ytitle({stSerif:Percent}) xtitle({stSerif:E at Maximum} {&rho}) note({stSerif:Note: Differencing at d`diff'}) ///
graphregion(color(white)) `bbbb' name(Hist_Emax_rho_`var', replace) graphregion(color(white)) discrete xlabel(`eminimum'(1)`emaximum') width(1)
 
if "`exportplot'" != "" {
graph export Hist_Emax_rho_`var'.`exportplot', replace
}
 
hist min_mae_`var', percent ///
title({stSerif:SP: Min MAE per Panel ID for `var'}, color(black)) ///
ytitle({stSerif:Percent}) xtitle({stSerif:Minimum MAE}) ///
note({stSerif:Note: Grand Mean of MAE at E That Minimizes MAE = `maeGM1`var'' -- differencing at d`diff'}) ///
graphregion(color(white)) `bbbbb' name(Hist_min_mae_`var', replace) graphregion(color(white))

if "`exportplot'" != "" {
graph export Hist_min_mae_`var'.`exportplot', replace
}

hist emin_mae_`var', percent ///
title({stSerif:SP: E at Min MAE per Panel ID for `var'}, color(black)) ///
ytitle({stSerif:Percent}) xtitle({stSerif:E at Minimum MAE}) note({stSerif:Note: Differencing at d`diff'}) ///
graphregion(color(white)) `bbbbbb' name(Hist_Emin_mae_`var', replace) graphregion(color(white)) discrete xlabel(`eminimum'(1)`emaximum') width(1)

if "`exportplot'" != "" {
graph export Hist_Emin_mae_`var'.`exportplot', replace
}
...
gen max_rho_`var'=.
label variable max_rho_`var' "The rho using `Ecriterion' criterion per Panel ID at d`diff'"
gen emax_rho_`var'=.
label variable emax_rho_`var' "The E using `Ecriterion' criterion per Panel ID at d`diff'"
capture {
if "`Ecriterion'"=="R2" {
...
* Plots for the R2 or proportion change in R2 case (dR2)
if "`SPgraph'"=="yes" & "`plots_on'" =="yes" {
if "`SPdescribe'"=="yes" {
foreach var of varlist `variables' {
* Saving results
if "`SPsave'"=="yes" {
loc b="saving(Histogram_rho_using_`Ecriterion'_`var'_d`diff', replace)"
loc bb="saving(Histogram_E_using_`Ecriterion'_`var'_d`diff', replace)"
}
else {
loc b=""
loc bb=""
...
capture {
hist max_rho_`var', percent ///
title({stSerif:SP: Selected} {&rho} {stSerif: Using `Ecriterion' Criterion per Panel for `var'}, color(black)) ///
ytitle({stSerif:Percent}) xtitle({&rho} {stSerif:Using `Ecriterion' Criterion to Select E}) ///
note({stSerif:Note: Grand Mean of} {&rho} {stSerif:at Selected E = `rhoGM1`var'' -- differencing at d`diff'}) ///
graphregion(color(white)) `b' name(Hist_rho_`Ecriterion'_`var', replace) graphregion(color(white))

if "`exportplot'" != "" {
graph export Hist_rho_`Ecriterion'_`var'.`exportplot', replace
}

hist emax_rho_`var', percent ///
title({stSerif:SP: Selected E Using `Ecriterion' Criterion per Panel for `var'}, color(black)) ///
ytitle({stSerif:Percent}) xtitle({stSerif:E Using `Ecriterion' Criterion to Select E}) note({stSerif:Note: Differencing at d`diff'}) ///
graphregion(color(white)) `bb' name(Hist_E_`Ecriterion'_`var', replace) graphregion(color(white)) discrete xlabel(`eminimum'(1)`emaximum') width(1)

if "`exportplot'" != "" {
graph export Hist_E_`Ecriterion'_`var'.`exportplot', replace
}
...
qui replace rhosign`var'=-1 if SP_rho_`var'<0
gen max_mae_`var'=.
label variable max_mae_`var' "The mae using `Ecriterion' criterion per Panel ID at d`diff'"
gen emax_mae_`var'=.
label variable emax_mae_`var' "The E using `Ecriterion' criterion per Panel ID at d`diff'"
...
* Plots for proportional change in MAE (dMAE) case
if "`SPgraph'"=="yes" & "`plots_on'" =="yes" {
if "`SPdescribe'"=="yes" {
foreach var of varlist `variables' {
* Saving results
if "`SPsave'"=="yes" {
loc b="saving(Histogram_mae_using_`Ecriterion'_`var'_d`diff', replace)"
loc bb="saving(Histogram_E_using_`Ecriterion'_`var'_d`diff', replace)"
}
else {
loc b=""
...
capture {
qui mean max_mae_`var'
mat `MaeGM'=e(b)
loc maeGM`var'=round(`MaeGM'[1,1], .01)
loc maeGM1`var': di %3.2f `maeGM`var''
}

hist max_mae_`var', percent ///
title({stSerif:SP: Plot of MAE} {stSerif: Using `Ecriterion' Criterion per Panel ID for `var'}, color(black)) ///
ytitle({stSerif:Percent}) xtitle({stSerif:MAE Using `Ecriterion' Criterion to Select E}) ///
note({stSerif:Note: Grand Mean of MAE at Selected E = `maeGM1`var'' -- differencing at d`diff'}) ///
graphregion(color(white)) `b' name(Hist_mae_`Ecriterion'_`var', replace) graphregion(color(white))

if "`exportplot'" != "" {
graph export Hist_mae_`Ecriterion'_`var'.`exportplot', replace
}

hist emax_mae_`var', percent ///
title({stSerif:SP: Selected E Using `Ecriterion' Criterion per Panel for `var'}, color(black)) ///
ytitle({stSerif:Percent}) xtitle({stSerif:E Using `Ecriterion' Criterion to Select E}) note({stSerif:Note: Differencing at d`diff'}) ///
graphregion(color(white)) `bb' name(Hist_E_`Ecriterion'_`var', replace) graphregion(color(white)) discrete xlabel(`eminimum'(1)`emaximum') width(1)
...
qui bysort `panelvar' (`timevar'): replace ``var'_nonzero' = 0 if `var' == 0
qui bysort `panelvar' (`timevar'): replace ``var'_nonzero' = sum(``var'_nonzero')
sort `panelvar' `timevar'
foreach id of numlist `panelrange' {
qui by `panelvar' (`timevar'): replace ``var'_nonzerouse' = 1 if ``var'_nonzero'[_n] < ``var'_nonzero'[_n-`emax`var'`id''-`diff']
}
tsset
...
bysort `panelvar': replace `randoM'=uniform()
if "`dropzeros_SP'"=="yes" {
bysort `panelvar' (``var'_nonzeroSPuse' `randoM'): replace `newtimE'=_n if ``var'_nonzeroSPuse'==1
bysort `panelvar' (``var'_nonzeroSPuse' `timevar'): replace `r`var''=`d`diff'`var''[`newtimE'] if ``var'_nonzeroSPuse'==1
tsset
}
else {
bysort `panelvar' (`randoM'): replace `newtimE'=_n
bysort `panelvar' (`timevar'): replace `r`var''=`d`diff'`var''[`newtimE']
tsset
}
foreach id of numlist `panelrange' {
if "`emax`var'`id''"!="" {
if `emax`var'`id''>0 {
di "----------------------------------------------------------"
di "Panel ID == `id', Rep == `i' of `SPtestreps', Variable == `var'"
di "----------------------------------------------------------"
capture {
qui edm explore `prestd'`r`var'' if `panelvar'==`id' ``var'dropzes', e(`emax`var'`id'') full `forcing' `time_gap' `missings' extra(`extras`diff'' `extras_raw') `tweight' `mdistance' reportrawe `options'
}
capture {
mata: `Clong'=`id'
...
label variable SPtestl`var'1 "Simplex Projection test (all reps), panel ID from `panelvar'"
...
title({stSerif:SP: `var' Confidence Intervals Centered on Permuted} {`z'}, color(black)) xtitle({stSerif:Panel ID from `panelvar'}) ytitle({stSerif:Prediction Accuracy} {`z'}) ///
...
yline(0) ytitle({stSerif:Prediction Accuracy} {`z'}) xtitle({stSerif:Panel ID from `panelvar'}) title({stSerif:SP: `var' Confidence Intervals Centered on Permuted} {`z'}, color(black)) ///
...
label variable TP`var'1 "Test of prediction tp steps ahead (TP), ID from `panelvar'"
...
label variable TPlong`var'1 "Simplex Projection long (all `TPsampler's), panel ID from `panelvar'"
...
expand 2
foreach var of varlist `variables' {
qui replace TP_id_`var' = 10^9 if _n > _N/2 & !missing(TP_id_`var')
}
foreach var of varlist `variables' {
* Confidence intervals for full plots
if `CIs'==95 loc zci=1.96
...
if `CIs'==95 | `CIs'==90 {
loc a="err(collapse(cilower`var' ciupper`var'))"
qui gen TP_`useresults'SD_`var'var=TP_`useresults'SD_`var'^2
qui bysort TP_id_`var' TP_e_`var': egen cilower`var' = mean(TP_`useresults'SD_`var'var)
qui bysort TP_id_`var' TP_e_`var': egen ciupper`var' = mean(TP_`useresults'SD_`var'var)
qui replace cilower`var' = TP_`useresults'_`var' - sqrt(cilower`var')*`zci'
qui replace ciupper`var' = TP_`useresults'_`var' + sqrt(ciupper`var')*`zci'
}
else loc a=""
* Confidence intervals for grand means
if `GMCIs'==95 loc zgmci=1.96
...
if `GMCIs'==95 | `GMCIs'==90 {
loc gmci="err(collapse(cilower`var'gm ciupper`var'gm))"
qui gen TP_`useresults'SD_`var'vargm=TP_`useresults'SD_`var'^2
qui bysort TP_id_`var' TP_e_`var': egen cilower`var'gm = mean(TP_`useresults'SD_`var'vargm)
qui bysort TP_id_`var' TP_e_`var': egen ciupper`var'gm = mean(TP_`useresults'SD_`var'vargm)
qui replace cilower`var'gm = TP_`useresults'_`var' - sqrt(cilower`var'gm)*`zgmci'
qui replace ciupper`var'gm = TP_`useresults'_`var' + sqrt(ciupper`var'gm)*`zgmci'
}
else loc gmci=""
* Use rho or MAE labeling scheme
if "`useresults'" == "rho" loc z="&rho"
...
* Saving results
if "`TPsave'"=="yes" {
loc b="saving(TP_`var'_`useresults'_d`diff', replace)"
loc gmb="saving(TP_`var'_`useresults'_d`diff'_gm, replace)"
}
else {
loc b=""
loc gmb=""
...
if "`grandmeanonly'"=="yes" {
* Plot command
lgraph TP_`useresults'_`var' TP_tp_`var' TP_id_`var' if TP_id_`var'==1000000000, ///
`gmci' `gmb' separate(.001) fit(`TPlinefit') `ylabelscale' ///
title({stSerif:TP: Grand-Mean Results for `var'}, color(black)) ///
...
loptions(1000000000 lwidth(*6)) foptions(1000000000 lwidth(*6)) graphregion(color(white)) bgcolor(white) ///
name(TP_`var'_`useresults'_d`diff'_gm, replace) nolegend xlabel(1(1)`TPmax')

if "`exportplot'" != "" {
graph export TP_`var'_`useresults'_d`diff'_gm.`exportplot', replace
}
...
* Plot command
lgraph TP_`useresults'_`var' TP_tp_`var' TP_id_`var', ///
...
label variable SMAP`var'1 "SMAP results, ID from `panelvar'"
...
label variable SMAPlong`var'1 "SMAP long results, ID from `panelvar'"
...
expand 2
foreach var of varlist `variables' {
qui replace SMAP_id_`var' = 10^9 if _n > _N/2 & !missing(SMAP_id_`var')
}
foreach var of varlist `variables' {
* Confidence intervals for full plots
if `CIs'==95 loc zci=1.96
...
if `CIs'==95 | `CIs'==90 {
loc a="err(collapse(cilower`var' ciupper`var'))"
qui gen SMAP_`useresults'SD_`var'var=SMAP_`useresults'SD_`var'^2
qui bysort SMAP_id_`var' SMAP_e_`var': egen cilower`var' = mean(SMAP_`useresults'SD_`var'var)
qui bysort SMAP_id_`var' SMAP_e_`var': egen ciupper`var' = mean(SMAP_`useresults'SD_`var'var)
qui replace cilower`var' = SMAP_`useresults'_`var' - sqrt(cilower`var')*`zci'
qui replace ciupper`var' = SMAP_`useresults'_`var' + sqrt(ciupper`var')*`zci'
}
else loc a=""
* Confidence intervals for grand means
if `GMCIs'==95 loc zgmci=1.96
...
if `GMCIs'==95 | `GMCIs'==90 {
loc gmci="err(collapse(cilower`var'gm ciupper`var'gm))"
qui gen SMAP_`useresults'SD_`var'vargm=SMAP_`useresults'SD_`var'^2
qui bysort SMAP_id_`var' SMAP_e_`var': egen cilower`var'gm = mean(SMAP_`useresults'SD_`var'vargm)
qui bysort SMAP_id_`var' SMAP_e_`var': egen ciupper`var'gm = mean(SMAP_`useresults'SD_`var'vargm)
qui replace cilower`var'gm = SMAP_`useresults'_`var' - sqrt(cilower`var'gm)*`zgmci'
qui replace ciupper`var'gm = SMAP_`useresults'_`var' + sqrt(ciupper`var'gm)*`zgmci'
}
else loc gmci=""
* Use rho or MAE labeling scheme
if "`useresults'" == "rho" loc z="&rho"
...
* Saving results
if "`SMAPsave'"=="yes" {
loc b="saving(SMAP_`var'_`useresults'_d`diff', replace)"
loc gmb="saving(SMAP_`var'_`useresults'_d`diff'_gm, replace)"
}
else {
loc b=""
loc gmb=""
...
if "`grandmeanonly'"=="yes" {
*Plot command
lgraph SMAP_`useresults'_`var' SMAP_theta_`var' SMAP_id_`var' if SMAP_id_`var'==1000000000, ///
`gmci' `gmb' separate(.001) fit(`SMAPlinefit') `ylabelscale' ///
title({stSerif:SMAP: Grand-Mean Results for `var'}, color(black)) ///
...
loptions(1000000000 lwidth(*6)) foptions(1000000000 lwidth(*6)) graphregion(color(white)) bgcolor(white) ///
name(SMAP_`var'_`useresults'_d`diff'_gm, replace) nolegend

if "`exportplot'" != "" {
graph export SMAP_`var'_`useresults'_d`diff'_gm.`exportplot', replace
}
...
* Plot command
lgraph SMAP_`useresults'_`var' SMAP_theta_`var' SMAP_id_`var', ///
...
* Identify and plot theta with best predictions
qui gen long `obsnO'=_n
foreach var of varlist `variables' {
qui gen ThetaMax`var'=.
qui gen ThetaMax`useresults'`var'=.
label variable ThetaMax`var' "Best theta using `useresults' criterion, at d`diff'"
label variable ThetaMax`useresults'`var' "Value of `useresults' at best theta, at d`diff'"
foreach id of numlist `panelrange' {
capture {
quietly summarize SMAP_`testval'_`var' if SMAP_id_`var'==`id', meanonly
...
replace ThetaMax`var'=SMAP_theta_`var'[`r(levels)'] if SMAP_id_`var'==`id'
replace ThetaMax`useresults'`var'=SMAP_`useresults'_`var'[`r(levels)'] if SMAP_id_`var'==`id'
}
}
tab ThetaMax`var' ThetaMax`useresults'`var'
sum ThetaMax`var' ThetaMax`useresults'`var'
* Saving results
if "`SPsave'"=="yes" {
loc b="saving(SMAP_scatter_`var'_`useresults'_d`diff', replace)"
loc bb="saving(SMAP_hist_`var'_`useresults'_d`diff', replace)"
}
else {
loc b=""
loc bb=""
...
capture {
qui mean ThetaMax`var'
tempname ThetaGM
mat `ThetaGM'=e(b)
loc thetaGM`var'=`ThetaGM'[1,1]
loc thetaGM`var': di %6.4f `thetaGM`var''

qui cor ThetaMax`var' ThetaMax`useresults'`var'
loc regrho5 = round(r(rho), .01)
loc regrho55: di %3.2f `regrho5'
}

scatter ThetaMax`var' ThetaMax`useresults'`var' || lfit ThetaMax`var' ThetaMax`useresults'`var', ///
title({stSerif:SMAP: Relationship of Optimal} {&theta} {stSerif:and} {`z'} {stSerif:per Panel ID for `var'}, color(black)) ///
ytitle({stSerif:Optimal} {&theta}) xtitle({`z'} {stSerif:Under Optimal} {&theta}) ///
graphregion(color(white)) name(SMAP_scatter_`var'_`useresults'_d`diff', replace) ///
legend(off) lcolor(black) lwidth(.75) `b' note("{stSerif:Note: Each point is a panel ID, r = `regrho55' -- differencing at d`diff'}")

if "`exportplot'" != "" {
graph export SMAP_scatter_`var'_`useresults'_d`diff'.`exportplot', replace
}

hist ThetaMax`var', percent ///
title({stSerif:SMAP: Optimal} {&theta} {stSerif:per Panel ID for `var'}, color(black)) ///
ytitle({stSerif:Percent}) xtitle({stSerif:Optimal} {&theta}) ///
note({stSerif:Note: Grand Mean of Optimal} {&theta} {stSerif:= `thetaGM`var'' -- differencing at d`diff'}) ///
graphregion(color(white)) `bb' name(SMAP_hist_`var'_`useresults'_d`diff', replace) graphregion(color(white)) discrete

if "`exportplot'" != "" {
graph export SMAP_hist_`var'_`useresults'_d`diff'.`exportplot', replace
}

drop ThetaMax`var'
drop ThetaMax`useresults'`var'
...
title({stSerif:SMAP: `var' Confidence Intervals of Difference in} {`z'}, color(black)) xtitle({stSerif:Panel ID from `panelvar'}) ytitle({stSerif:Prediction Accuracy} {`z'}) ///
...
yline(0) ytitle({stSerif:Prediction Accuracy} {`z'}) xtitle({stSerif:Panel ID from `panelvar'}) title({stSerif:SMAP: `var' Confidence Interval of Difference in} {`z'}, color(black)) ///
...
di "-------------------------------------------------"
di "Below ID==`id', predictor==`var1', predictee==`var2'"
di "-------------------------------------------------"
capture {
if "`emax`var1'`id''"!="" {
if `emax`var1'`id''>0 {
...
if `emax`var1'`id''>0 {
di "-------------------------------------------------"
di "Below ID==`id', CCM Cause==`var1', Outcome==`var2'"
di "-------------------------------------------------"
...
label variable ID_`var1'_pred_`var2'_CCM1 "CCM `var1' causing `var2', ID from `panelvar'"
...
foreach var2 of varlist `variables' {
if "`var1'" != "`var2'" {
qui replace CCM_id_`var1'_`var2' = 10^9 if _n > _N/2 & !missing(CCM_id_`var1'_`var2')
}
}
}
* Use rho or MAE labeling scheme
...
loc bb="saving(CCM_`var1'_`var2'_`useresults'_l0gm, replace)"
}
else {
loc b=""
loc bb=""
...
if CCM_lag_`var1'_`var2'==0 & !missing(CCM_`useresults'_`var1'_`var2') & CCM_id_`var1'_`var2'==1000000000, ///
/// errortype(ci(`GMCIs'))
`bb' separate(.001) fit(`CCMlinefit') `ylabelscale' ///
title({stSerif:CCM: Grand-mean results for `var1' CCM causing `var2'}, color(black)) ///
...
loptions(1000000000 lwidth(*6)) foptions(1000000000 lwidth(*6)) graphregion(color(white)) bgcolor(white) ///
name(CCM_`var1'_`var2'_`useresults'_l0gm, replace) nolegend /// 
note({stSerif:Note: Grand Mean of} {&rho} {stSerif:at Maximum Library Size = `CCMrhoz11' -- differencing at d`diff'})

if "`exportplot'" != "" {
graph export CCM_`var1'_`var2'_`useresults'_l0gm.`exportplot', replace
}
...
* Saving results
if "`CCMlagsave'"=="yes" {
loc b="saving(CCM_`var1'_`var2'_`useresults'_l, replace)"
loc bb="saving(CCM_`var1'_`var2'_`useresults'_l_gm, replace)"
}
else {
loc b=""
loc bb=""
...
if !missing(CCM_`useresults'_`var1'_`var2') & CCM_id_`var1'_`var2'==1000000000 ///
& CCM_L_`var1'_`var2'max==CCM_L_`var1'_`var2', ///
/// errortype(ci(`GMCIs'))
`bb' separate(.001) fit(`CCMlaglinefit') `ylabelscale' ///
title({stSerif:CCM: Grand-mean lagged results for `var1' CCM causing `var2'}, color(black)) ///
ytitle({stSerif:Prediction Accuracy} {`z'}) xtitle({stSerif:Time Lag (Negative is Past)}) ///
note({stSerif:Note: Grand-mean line is bolded -- differencing at d`diff'}) ///
loptions(1000000000 lwidth(*6)) foptions(1000000000 lwidth(*6)) graphregion(color(white)) bgcolor(white) ///
name(CCM_`var1'_`var2'_`useresults'_l_gm, replace) nolegend xlabel(-`CCMlags'(1)`CCMleads')

if "`exportplot'" != "" {
graph export CCM_`var1'_`var2'_`useresults'_l_gm.`exportplot', replace
}
...
note({stSerif:Note: Grand-mean line is bolded -- differencing at d`diff'}) ///
loptions(1000000000 lwidth(*6)) foptions(1000000000 lwidth(*6)) graphregion(color(white)) bgcolor(white) ///
name(CCM_`var1'_`var2'_`useresults'_l, replace) nolegend xlabel(-`CCMlags'(1)`CCMleads')

if "`exportplot'" != "" {
graph export CCM_`var1'_`var2'_`useresults'_l.`exportplot', replace
}
...
di "-------------------------------------------------------------------------------------------"
di "Below ID==`id', Reps are `newCCMctestreps' for each ID, CCM Cause==`var1', Outcome==`var2'"
di "-------------------------------------------------------------------------------------------"
capture {
...
if "`CCMsave'" == "yes" loc savers1="saving(CCMctest_`var1'_`var2'_`testval'1, replace)"
if "`CCMsave'" == "yes" loc savers2="saving(CCMctest_`var1'_`var2'_`testval'2, replace)"
...
title({stSerif:CCM: Convergence test of `var1' CCM causing `var2'}, color(black)) xtitle({stSerif:Panel ID from `panelvar'}) ytitle({stSerif:Prediction Accuracy} {`z'}) ///
...
yline(0) ytitle({stSerif:Prediction Accuracy} {`z'}) xtitle({stSerif:Panel ID from `panelvar'}) title({stSerif:CCM: Convergence test of `var1' CCM causing `var2'}, color(black)) ///
...
bysort `panelvar': replace `randoM'=uniform()
if "`dropzeros'"=="yes" {
bysort `panelvar' (``var1'_nonzerouse' `randoM'): replace `newtimE'=_n if ``var1'_nonzerouse'==1
bysort `panelvar' (``var1'_nonzerouse' `timevar'): replace `r`var1''=`d`diff'`var1''[`newtimE'] if ``var1'_nonzerouse'==1
tsset
}
else {
bysort `panelvar' (`randoM'): replace `newtimE'=_n
bysort `panelvar' (`timevar'): replace `r`var1''=`d`diff'`var1''[`newtimE']
tsset
}
...
di "--------------------------------------------------------------------------------"
di "Below ID==`id', Rep == `i' of `CCMLtestreps', CCM Cause==`var1', Outcome==`var2'"
di "--------------------------------------------------------------------------------"
capture {
...
legend(label(1 {stSerif:Permuted Mean Diff}) label(2 {stSerif:`fivepct'th Pctile Diff}) label(3 {stSerif:`onepct'`onesuffix' Pctile Diff})) ///
title({stSerif:CCM: Test of `var1' CCM causing `var2' at max L}, color(black)) xtitle({stSerif:Panel ID from `panelvar'}) ytitle({stSerif:Prediction Accuracy} {`z'}) ///
...
yline(0) ytitle({stSerif:Prediction Accuracy} {`z'}) xtitle({stSerif:Panel ID from `panelvar'}) title({stSerif:CCM: Test of `var1' CCM causing `var2' at max L}, color(black)) ///
...
di "--------------------------------------------------------------------------------"
di "CCM SMAP  Below ID==`id', Cause==`var1', Outcome==`var2'"
di "--------------------------------------------------------------------------------"
loc a = 0
capture noisily {
...
putexcel A11 = "For N=1 or 'multispatial EDM' with N>1 cases this is either non-significant and 0 or significant and 100"
